...
(async () => {
  try {
    ...
    const backend = new UltraPlonkBackend(circuit.bytecode, { threads: 1 });
    ...
    // save proof to file
    fs.writeFileSync(`../semaphore/target/proof-clean-ultra-depth${depth}`, proof);
    ...
    console.log("Proof generated and saved to ../semaphore/target/proof-clean-ultra-depth${depth}");
  } catch (error) {
    console.error(error);
  }
})();
