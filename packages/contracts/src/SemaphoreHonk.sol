...
import {ISemaphoreHonk} from "./interfaces/ISemaphoreHonk.sol";
...
contract SemaphoreHonk is ISemaphoreHonk, SemaphoreGroups {
    ...
    function validateProof(uint256 groupId, SemaphoreHonkProof calldata proof) external override {
    ...
    function verifyProof(
        uint256 groupId,
        SemaphoreHonkProof calldata proof
        ...
        publicInputs[0] = bytes32(proof.merkleTreeRoot);
        publicInputs[1] = bytes32(proof.nullifier);
        publicInputs[2] = bytes32(_hash(proof.message));
        publicInputs[3] = bytes32(_hash(proof.scope));
