...
import "../interfaces/HonkVerificationKey.sol";
import {HonkVerificationKeys} from "./HonkVerificationKeys.sol";
...
abstract contract BaseHonkVerifier is IHonkVerifier {
    using FrLib for Fr;
    ...
    function loadVerificationKey(uint256 treeDepth) internal view virtual returns (Honk.VerificationKey memory);

    function verify(bytes calldata proof, bytes32[] calldata publicInputs, uint256 treeDepth) public view override returns (bool) {
        console.log("BaseHonkVerifier.verify");
        Honk.VerificationKey memory vk = loadVerificationKey(treeDepth);
        ...
        t.relationParameters.publicInputsDelta = computePublicInputDelta(
            publicInputs, t.relationParameters.beta, t.relationParameters.gamma, p.publicInputsOffset, vk.circuitSize
        );
    ...
    function computePublicInputDelta(bytes32[] memory publicInputs, Fr beta, Fr gamma, uint256 offset, uint256 n)
        internal
        view
    ...
    function verifySumcheck(Honk.Proof memory proof, Transcript memory tp, uint256 logN) internal view returns (bool verified) {
        Fr roundTarget;
        ...
        returns (bool verified)
    {
        ...
        Fr[CONST_PROOF_SIZE_LOG_N + 1] memory inverse_vanishing_evals =
            CommitmentSchemeLib.computeInvertedGeminiDenominators(tp.shplonkZ, powers_of_evaluation_challenge, vk.logCircuitSize);
...
contract SemaphoreHonkVerifier is BaseHonkVerifier(NUMBER_OF_PUBLIC_INPUTS) {
    HonkVerificationKeys public verificationKeys;
    constructor(address _verificationKeys) {
        verificationKeys = HonkVerificationKeys(_verificationKeys);
    }

    function loadVerificationKey(uint256 treeDepth) internal view override returns (Honk.VerificationKey memory) {
        console.log("loading verification key for tree depth %s", treeDepth);
        return verificationKeys.loadVerificationKey(treeDepth);
    }
}
